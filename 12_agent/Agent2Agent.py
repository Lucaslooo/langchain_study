...
def creat_ctrip_agent(llm):
    llm_with_tools = llm.bind_tools([ctrip_book_flight])
    prompt = ChatPromptTemplate.from_messages(
        [
            ("system", "你是专业的调用工具，你只能调用CtripBookFlight工具，"
                       "调用格式必须正确,"
                       "直接传入参数:departure='北京',arrival='上海',date='2026-04-15',"
                       "调用后直接返回工具执行的字符串完整结果，不能有其它内容，不能留空"),
            ("human", "{input}")
        ]
    )
...
def create_travel_coordinator_agent(llm, ctrip_chain, meituan_chain, didi_chain):
    ...
    return RunnableLambda(a2a_schedule)   #把任意普通函数 → 变成 LangChain 可调用的 “可运行对象”
...
if __name__ == "__main__":
    try:
        ...
        ctrip_chain = creat_ctrip_agent(llm)
