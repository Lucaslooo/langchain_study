...
子图状态（SubgraphState）：包含 analysis_input（分析输入）、analysis_result（分析结果）、intermediate_steps（中间步骤），聚焦分析层；
两者无重叠字段，完全独立，必须通过代理节点手动转换。
...
核心方案：父子图状态不同时，通过父图的代理节点而非直接添加子图节点，手动完成「父状态→子输入」转换、调用子图、「子输出→父状态」映射；
关键要点：代理节点必须接收父图状态、返回父图状态，子图调用通过 compiled_subgraph.invoke() 手动触发；
灵活性：该模式可适配任意结构的父子图状态，只需在代理节点中自定义转换逻辑，是 LangGraph 中跨图状态交互的标准做法。
'''
...
def build_subgraph() -> StateGraph:
    ...
    sub_builder.add_node("analysis_node", subgraph_analysis_node)
    sub_builder.add_edge(START, "analysis_node")
    sub_builder.add_edge("analysis_node", END)
...
def call_subgraph_proxy(state: ParentState) -> ParentState:
    ...
    """
    # 步骤1：父图状态 → 子图输入（状态转换）
    # 提取父图的user_query，转换为子图需要的analysis_input
    subgraph_input = {
