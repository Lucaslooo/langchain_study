...
Ø langgraph-checkpoint-sqlite：使用SQLite数据库（SqliteSaver / AsyncSqliteSaver）存储检查点。
非常适合实验和本地工作流程。需要单独安装。
Ø langgraph-checkpoint-postgres：使用Postgres数据库（PostgresSaver / AsyncPostgresSaver）
...
def main():
    ...
    conn = sqlite3.connect(database="D:\\sql_lite_test\\sqlite_data.db", check_same_thread=False)
