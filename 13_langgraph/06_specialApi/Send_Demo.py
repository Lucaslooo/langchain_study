...
class AtguiguState(TypedDict):
    subjects: List[str]
    jokes: Annotated[List[str], lambda x, y: x + y]  # jokes：带合并规则的列表;lambda x,y: x+y 的意思：新结果直接追加到老结果后面，实现自动合并
...
# Map节点：为每个主题生成笑话
"""
LangGraph 里的 Map 节点（并行遍历节点）：
你的 state 里存的是 subjects 列表：["猫", "狗", "程序员"]
当框架用 Map 模式 运行 make_joke 时，
它会自动把列表里的每一项单独拆出来，临时放进 state 里，key 叫 subject（单数）
"""
def make_joke(state: AtguiguState) -> dict:
    """为单个主题生成笑话"""
    subject = state.get("subject", "未知")  #这里的 subject 不是来自你定义的状态，而是来自 Send 传进去的临时值！
